...
export default function Sidebar({ isGuest = false, guestBranch = '', guestSepType = '' }) {
  ...
  return (
    <div style={{
      ...
      {/* Branch color strip */}
      <div style={{ height: '3px', backgroundColor: bc.color }} />

      {/* Logo + branch tag */}
      <div style={{ padding: '16px 16px 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
          <div style={{ width: '32px', height: '32px', borderRadius: '8px', backgroundColor: bc.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>🎖️</div>
          <span style={{ fontWeight: '700', fontSize: '15px', color: '#111', letterSpacing: '-0.3px' }}>MilSep</span>
        </div>
        {branch && (
          <>
            <div style={{ backgroundColor: bc.badge, borderRadius: '6px', padding: '4px 10px', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
              <span style={{ fontSize: '12px' }}>{bc.symbol}</span>
              <span style={{ color: bc.text, fontSize: '10px', fontWeight: '700', letterSpacing: '0.5px' }}>{branch.toUpperCase()}</span>
            </div>
      ...
      <nav style={{ padding: '8px', flex: 1 }}>
        {navItems.map(item => (
          <div key={item.path} onClick={() => nav(item.path)} style={{
